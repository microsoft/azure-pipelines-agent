    ...
    public sealed class MessageListenerL0 : IDisposable
    {
        ...
        public async void CreateSessionUsesExponentialBackoffWhenFlagEnabled()
        {
            ...
            using (var tokenSource = new CancellationTokenSource())
            {
                ...
                var enabledFlag = new FeatureAvailability.FeatureFlag("", "", "", "On", "On");
                _featureFlagProvider
                    .Setup(x => x.GetFeatureFlagAsync(
                        It.IsAny<IHostContext>(), 
                        "DistributedTask.Agent.EnableProgressiveRetryBackoff",
                        It.IsAny<ITraceWriter>(), 
                        It.IsAny<CancellationToken>()))
                    .Returns(Task.FromResult(enabledFlag));
        ...
        public async void CreateSessionUsesConstantBackoffWhenFlagDisabled()
        {
            ...
            using (var tokenSource = new CancellationTokenSource())
            {
                ...
                // Arrange - Feature flag is OFF
                var disabledFlag = new FeatureAvailability.FeatureFlag("", "", "", "Off", "Off");
                _featureFlagProvider
                    .Setup(x => x.GetFeatureFlagAsync(
                        It.IsAny<IHostContext>(), 
                        "DistributedTask.Agent.EnableProgressiveRetryBackoff",
                        It.IsAny<ITraceWriter>(), 
                        It.IsAny<CancellationToken>()))
                    .Returns(Task.FromResult(disabledFlag));

    ...
    internal static partial class AdditionalMaskingRegexes
    {
        ...
        private static IEnumerable<string> credScanPatterns =
            new List<string>()
            {
                ...
                // JsonWebToken
                @"(?<=(?-i))"
                + @"(?<JwtToken>eyJ(?i)[a-z0-9\-_%]+\.(?-i)eyJ(?i)[a-z0-9\-_%]+\.[a-z0-9\-_%]+)"
                + @"(?=|([rR]efresh_?[tT]oken|REFRESH_?TOKEN)[""']?\s*[:=]{1,2}\s*[""']?(?<JwtToken>(\w+-)+\w+)[""']?)",

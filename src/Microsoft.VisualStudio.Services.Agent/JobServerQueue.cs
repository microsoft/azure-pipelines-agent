...
namespace Microsoft.VisualStudio.Services.Agent
{
    ...
    public sealed class JobServerQueue : AgentService, IJobServerQueue
    {
        ...
        public async Task QueueTimelineRecordUpdateAsync(Guid timelineId, TimelineRecord timelineRecord)
        {
            ...
            if (timelineRecord.State == TimelineRecordState.InProgress && timelineId == _jobTimelineId && timelineRecord.Id == _jobTimelineRecordId)
            {
            ...
            // Queue the update normally for non-critical updates or if immediate update failed
            _timelineUpdateQueue.TryAdd(timelineId, new ConcurrentQueue<TimelineRecord>());
            Trace.Verbose("Enqueue timeline {0} update queue: {1}", timelineId, timelineRecord.Id);
            _timelineUpdateQueue[timelineId].Enqueue(timelineRecord.Clone());
        }
